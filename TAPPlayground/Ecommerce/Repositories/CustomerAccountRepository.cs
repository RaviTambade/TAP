...
public class CustomerAccountRepository : ICustomerAccountRepository
{
    ...
    public bool GetById()
    {
        ...
        try
        {
            string query = "select customers.cust_id,customers.first_name,customers.last_name,customers.email,customers.contact_number,accounts.account_number,accounts.ifsc_code,accounts.balance FROM customers  INNER JOIN  accounts ON accounts.account_number=customers.account_number WHERE account_number=@accountNumber";
            MySqlCommand command = new MySqlCommand(query, con);
            con.Open();
            MySqlDataReader reader = command.ExecuteReader();
            while (reader.Read())
