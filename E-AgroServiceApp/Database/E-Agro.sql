...
CREATE TABLE
    farmerPurchasesBilling(
        ...
        totalAmount DOUBLE AS ( (amount1) + (amount2) + (amount3) + (amount4) + (amount5)
        ),
...
FOR EACH ROW BEGIN 
	...
	VALUES (NEW.purchaseId, NEW.netWeight);
END; 
...
EACH ROW BEGIN 
	DELETE FROM users WHERE contactNumber = OLD.contactNumber;
END; 
...
CREATE TRIGGER debit_balance AFTER INSERT ON farmerPurchasesBilling 
FOR EACH ROW BEGIN 
	UPDATE farmers
	SET
